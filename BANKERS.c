...
int main(int argc, char** argv) {
	...
	int i, j;
	 ...
	 printf("\n");
	 for( i=0; i<nProcesses; i++) {
		 printf("Enter the instances of each resource to be allocated to process %d:\n", i+1);
	...
	// maximum required resources
	 for( i=0; i<nProcesses; i++) {
		 printf("\nEnter the maximum instances of each resource required by process %d:\n", i+1);
		 for( j=0; j<nResources; j++){
			printf("R%d: ", j+1);
	 ...
	 printf("Enter instances of currently available resources:\n");
