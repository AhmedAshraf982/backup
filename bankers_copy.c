...
int main(int argc, char** argv) {
	...
	int i, j;
	 ...
	 printf("\n");
	 for( i=0; i<nProcesses; i++) {
		 printf("Enter instances of each resource allocated to process %d: ", i+1);
	...
	// maximum required resources
	 for( i=0; i<nProcesses; i++) {
		 printf("Enter max instances of each resource (R1, R2, ..) required by process %d: ", i+1);
		 for( j=0; j<nResources; j++){
		 	prinf("R#d: ", j+1);
		 	scanf("%d", &maxRequired[i][j]);
		 }
	 }
	 printf("Enter currently available instances of each resource(R1 R2 ...): ");
	...
	safeSeq = (int *)malloc(nProcesses * sizeof(*safeSeq));
	 for( i=0; i<nProcesses; i++) safeSeq[i] = -1;
		 if(!getSafeSeq()) {
			 printf("Error! The processes leads the system to an unsafe state.\n");
		 exit(-1);
		 }
	 printf("\nSafe Sequence Found: ");
	 for( i=0; i<nProcesses; i++) {
