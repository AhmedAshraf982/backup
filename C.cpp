...
/*
The overall Algorithm briefly goes as:
 - Frist we detect a cycle in the given graph.
 - Then check if this particular cycle is somewhat a duplicate of previously detected cycles.
    - If yes, discard it.
 - After that, we reduce it to its simplest graph having minimal weights by Kruskal's Algorithm.
 - If it has some edges in common with previously added circuits, then combine it with that one.
 - Continue the algorithm for next cycles to be detected.
*/
using namespace std;
