...
def Floyd(no_of_nodes):
    ...
    parent = [-1]*nodes
    floydWarshMatrix = array([[inf]* nodes]*nodes)
    for i in range (nodes):
            for j in range(nodes):
                floydWarshMatrix[i,j] = adjMatrix[i,j]
    for i in range (nodes):
            for j in range(nodes):
                print(floydWarshMatrix[i,j], end = "  ")
            print()
    for i in range(nodes):
            for j in range (nodes):
                for k in range(nodes):
                    if (floydWarshMatrix[j,k] > floydWarshMatrix[j,i] + floydWarshMatrix[i,k]):
                            floydWarshMatrix[j,k] = floydWarshMatrix[j,i] + floydWarshMatrix[i,k]
                            parent[k] = i
                            parent[i] = j
